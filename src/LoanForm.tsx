...
const LoanForm = () => {
    ...
    return (
            <Form onSubmit={handleSubmit}>
                <FormGroup>
                    <Label size="lg"  htmlFor={'amount'}> {'💵 Amount '} </Label>
                    <Input bsSize="lg"  name={'amount'} {...bindAmount} type="number"/>
                </FormGroup>
                <FormGroup>
                    <Label size="lg" htmlFor={'payback'}> {'📅 Payment time in years'} </Label>
                    <Input bsSize="lg"  {...bindPayback} type="number"/>
                </FormGroup>
                <Button color="primary" size="lg" block disabled={canGenerateSchedule(amountValue, paybackValue)}>
                    Calculate
                </Button>
            </Form>
    )
